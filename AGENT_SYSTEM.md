...
4. **Execute**: Utilize curated skills from `.agent/skills/`.
5. **Verify**: Test outputs against `.gsd/SPEC.md` requirements.
6. **Record Evidence**: Append bounded implementation evidence or bounded intake summaries to `.gsd/JOURNAL.md` when useful.
7. **Synchronize Canonical State**: Hand results, including any brainstorm or chat-ingestion outcomes that should become official, to the orchestrator or explicit GSD/governance pass to update `.gsd/STATE.md`, `.gsd/ROADMAP.md`, `.gsd/TODO.md`, and `.gsd/DECISIONS.md` safely.
8. **Commit**: Commit with GSD-style atomicity after the relevant state sync has occurred.

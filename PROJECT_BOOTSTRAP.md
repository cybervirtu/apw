...
- Vendors the shared downstream core command pack from the root `.agent/` source:
  - workflows: `/status`, `/brainstorm`, `/create`, `/enhance`, `/debug`, `/test`, `/orchestrate`
...
- `templates/` is the canonical profile source used for lifecycle and profile-specific execution-layer template content.
- `base` and `advanced` receive the shared downstream core command pack from the canonical root `.agent/` source in addition to any profile-local template content.
...
- the shared downstream core command pack for `base` and `advanced`

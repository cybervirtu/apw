...
5. **COMMIT**: One task = one commit. Verified implementation slices and completed phases should be committed promptly once verification and required state sync are complete. Format: `type(scope): description`. **Always utilize the provided `.gitmessage` template.**
...
- **Verified Completion Commit Rule**: When a bounded implementation slice is completed and verified, or when a phase is officially closed, capture that result in a git commit before unrelated work continues. If verification exposes more follow-up work, promote the follow-ups first, then commit the verified checkpoint.
...
- **Requirement Register Rule**: Official user requirements should be recorded in `.gsd/SPEC.md` as a lightweight requirement register, not left only in chat history. Each requirement should have a stable identifier, concise requirement statement, and lifecycle status so the team can trace what was requested, what is implemented, what was enhanced, and what is still pending.
- **Requirement Traceability Rule**: When practical, canonical work items and progress notes should reference requirement IDs. `.gsd/TODO.md` should point to the requirement IDs it advances, `.gsd/STATE.md` should describe current status against those IDs, and `.gsd/JOURNAL.md` should capture evidence or change history that helps verify them.
...
- **Project-Essential Markdown Commit Rule**: Commit and push only the Markdown files that are required to operate or understand the downstream project, such as canonical `.gsd/` files, project-specific `docs/`, and the APW contract files intentionally kept at repo root. Do not bulk-push the broader APW handbook, guideline, or reference Markdown set unless the repository is intentionally maintaining APW itself or performing a reviewed APW upgrade.
